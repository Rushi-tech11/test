...
const TaskItem = ({task,onRefresh,onLogout,showCompleteCheckbox = true}) => {
  ...
  const[isCompleted,setIsCompleted]=useState(
    [true,1,'yes'].includes(
      typeof task.isCompleted==='string'?task.isCompleted.toLowerCase():task.isCompleted
    )
  )
 const [showEditModal, setShowEditModal] = useState(false);
  ...
  useEffect(()=>{
    setIsCompleted(
      [true,1,'yes'].includes(
        typeof task.isCompleted==='string'?task.Completed.toLowerCase():task.isCompleted
      )
    )
  },[task.Completed])
 ...
 <div className={`${TI_CLASSES.wrapper}${borderColor}`}>
  <div className={TI_CLASSES.leftContainer}>
    ...
    <div className='flex-1 min-w-0'>
      <div className='flex items-baseline gap-2 mg-1 flex-wrap'>
    ...
    <div className={`${TI_CLASSES.dateRow} ${task.dueDate && isToday(new Date(task.dueDate))
      ...
      {task.dueDate ? (isToday(new Date(task.dueDate)) ? 'Today': format(new Date(task.dueDate),'MMM dd')) : '-'}
        ...
        <div className={TI_CLASSES.createdRow}>
          ...
          {task.createdAt?`Created ${format(new Date(task.createdAt),'MMM dd')}`:'No'}
