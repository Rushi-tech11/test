...
const Navbar = ({ user = {}, onLogout }) => {
  ...
  useEffect(()=>{
    const handleClickOutside=(event)=>{
      if(menuRef.current&& !menuRef.current.contain(event.target)){
        setMenuOpen(false)
      }
    }
    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200 font-sans">
      <div className="flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto">
        ...
        <div
          ...
          onClick={() => navigate("/")}
        >
          <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 via-purple-500 to-indigo-500 shadow-lg group-hover:shadow-purple-300/50 group-hover:scale-105 transition-all duration-300">
            <Zap className="w-6 h-6 text-white" />
          </div>

          <div className="absolute -bottom-1 left-7 w-3 h-3 bg-white rounded-full shadow-md animate-ping" />
        ...
        <div className="flex items-center gap-4">
          ...
          <div ref={menuRef} className="relative">
            ...
            >
              ...
              {/* Online Pulse */}
              <div className="justify-absolute  bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-white animate-pulse"></div>
              ...
              <ul className="absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden animate-fadeIn">
                ...
                <li className="p-2">
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-lg px-3 text-sm hover:bg-red-50 text-red-600 "
                    role="menuitem"
                  >
